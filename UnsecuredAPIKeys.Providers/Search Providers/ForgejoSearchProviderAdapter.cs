...
/// <summary>
/// Versioned Forgejo adapter (Phase 5, Task 22.2). Admin-approved self-hosted only.
...
/// </summary>
public sealed class ForgejoSearchProviderAdapter : ISearchProviderAdapter
{
    private readonly IHttpClientFactory? _httpClientFactory;
    private readonly HttpClient? _directClient;
    private readonly ISearchProviderOutcomeClassifier _classifier;

    public SearchProviderEnum ProviderKind => SearchProviderEnum.Forgejo;
    public string AdapterVersion => "forgejo-search-v1";
    ...
    public ProviderSettingsSchema SettingsSchema => new(1,
        [
            new ProviderSettingDefinition("ApiFlavor", "string", true, "Must be 'forgejo' (non-secret)."),
            new ProviderSettingDefinition("ServerVersion", "string", true, "Forgejo server version, e.g. '1.21.0' (non-secret).")
        ]);

    public ForgejoSearchProviderAdapter() : this(httpClientFactory: null, classifier: null) { }
    public ForgejoSearchProviderAdapter(HttpClient httpClient, ISearchProviderOutcomeClassifier? classifier = null)
    {
        _directClient = httpClient ?? throw new ArgumentNullException(nameof(httpClient));
        _classifier = classifier ?? new ForgejoResponseClassifier();
    }
    [ActivatorUtilitiesConstructor]
    public ForgejoSearchProviderAdapter(IHttpClientFactory? httpClientFactory, ISearchProviderOutcomeClassifier? classifier = null)
    {
    ...
    internal static bool IsSupportedVersion(string settingsJson, out string? sanitizedFlavor, out string? sanitizedVersion)
    {
        ...
        if (!string.Equals(sanitizedFlavor, "forgejo", StringComparison.OrdinalIgnoreCase)) return false;
    ...
    private HttpClient CreateClient(ValidatedProviderInstance instance)
    {
        ...
        var client = _httpClientFactory?.CreateClient("ForgejoSearch") ?? new HttpClient();
    ...
    public Task<ProviderCapabilityResult> DiscoverCapabilitiesAsync(ValidatedProviderInstance instance, CancellationToken cancellationToken)
    {
        ...
        if (instance.ProviderKind != SearchProviderEnum.Forgejo)
        ...
        using (response)
        {
            ...
            foreach (var item in arr)
            {
                ...
                results.Add(new ProviderResultInput(
                    ProviderKind: SearchProviderEnum.Forgejo,
